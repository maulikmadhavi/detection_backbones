...
from vitg.network.backbone.vitgyolov8.yolo.utils.plotting import colors, save_one_box
from vitg.network.backbone.vitgyolov8.yolo.v8.detect.predict import DetectionPredictor
...
class SegmentationPredictor(DetectionPredictor):
    def postprocess(self, preds, img, orig_img, classes=None):
        ...
        for i, pred in enumerate(p):
            ...
            if not len(pred):
                results.append(
                    Results(boxes=pred[:, :6], orig_shape=shape[:2])
                )  # save empty boxes
                continue
            if self.args.retina_masks:
                ...
                pred[:, :4] = ops.scale_boxes(img.shape[2:], pred[:, :4], shape).round()
            results.append(
                Results(boxes=pred[:, :6], masks=masks, orig_shape=shape[:2])
            )
        return results
