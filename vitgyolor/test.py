...
from vitg.network.backbone.vitgyolor.models.experimental import attempt_load
from vitg.network.backbone.vitgyolor.utils.datasets import create_dataloader
...
from vitg.network.backbone.vitgyolor.utils.loss import compute_loss
from vitg.network.backbone.vitgyolor.utils.metrics import ap_per_class
from vitg.network.backbone.vitgyolor.utils.plots import output_to_target, plot_images
...
def test(
    ...
    nc = 1 if single_cls else model.nc  # number of classes
    print("nc in test ")
    print(model.nc)
    ...
    names = [str(item) for item in range(82)]
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        if plots and batch_i < 100:
            ...
            plot_images(img, targets, [str(item) for item in paths], f, names)  # labels
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--weights", nargs="+", type=str, default="yolor-p6.pt", help="model.pt path(s)"
    )
    parser.add_argument(
