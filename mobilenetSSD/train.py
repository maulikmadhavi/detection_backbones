...
from src.model import SSD, MobileNetV2, ResNet, SSDLite
from src.process import evaluate, train
from src.transform import SSDTransformer
from src.utils import Encoder, coco_classes, generate_dboxes


def get_args():
    parser = ArgumentParser(description="Implementation of SSD")
    parser.add_argument(
        "--data-path", type=str, default="/coco", help="the root folder of dataset"
    )
    parser.add_argument(
        "--save-folder",
        type=str,
    ...
    parser.add_argument(
        "--epochs", type=int, default=65, help="number of total epochs to run"
    )
    parser.add_argument(
        "--batch-size",
        type=int,
    ...
    parser.add_argument(
        "--lr", type=float, default=2.6e-3, help="initial learning rate"
    )
    parser.add_argument(
        "--momentum",
        type=float,
...
def main(opt):
    ...
    train_set = CocoDataset(
        opt.data_path, 2017, "train", SSDTransformer(dboxes, (300, 300), val=False)
    )
    train_loader = DataLoader(train_set, **train_params)
    test_set = CocoDataset(
        opt.data_path, 2017, "val", SSDTransformer(dboxes, (300, 300), val=True)
    )
    ...
    for epoch in range(first_epoch, opt.epochs):
        train(
            model, train_loader, epoch, writer, criterion, optimizer, scheduler, opt.amp
        )
        # evaluate(model, test_loader, epoch, writer, encoder, opt.nms_threshold)
        evaluate_lmdb(
            model, test_loader, epoch, writer, encoder, opt.nms_threshold, opt.amp
        )
